...
describe("i18n 字典完整性", () => {
  ...
  it("九语键集完全一致(缺失/多余都算漂移)", () => {
    const zh = new Set(Object.keys(I18N.zh));
    for (const l of ["en", "ja", "de", "fr", "ko", "zh-Hant", "es", "pt-BR"]) {
  ...
  it("语言按钮代码/菜单为各语言自称(不走翻译)", () => {
    expect(LANG_CODES).toEqual({ zh: "中", en: "EN", ja: "あ", de: "DE", fr: "FR", ko: "한", "zh-Hant": "繁", es: "ES", "pt-BR": "BR" });
    expect(LANG_MENU.ja).toBe("日本語");
    expect(LANG_MENU.de).toBe("Deutsch");
    expect(LANG_MENU.fr).toBe("Français");
    expect(LANG_MENU.ko).toBe("한국어");
    expect(LANG_MENU["zh-Hant"]).toBe("中文(繁體)");
    expect(LANG_MENU.es).toBe("Español");
    expect(LANG_MENU["pt-BR"]).toBe("Português (Brasil)");
  });
});
