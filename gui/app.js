...
function _applyStatic() {
  ...
  document.title = ({ zh: "ASTBOX 容器管理器 · V3.0.1",
                      en: "ASTBOX Container Manager · V3.0.1",
                      ja: "ASTBOX コンテナマネージャー · V3.0.1" })[_lang]
                   || "ASTBOX 容器管理器 · V3.0.1";
...
function showAbout() {
  const sheet = openSheet(
    ...
    "<h2>" + _t("shAbout") + "</h2>" +
    '<p class="sheet-sub" style="text-align:center"><b>V3.0.1</b><br>' +
    _t("aboutBody") + '</p>' +
    '<div class="sheet-actions"><button class="btn btn-primary" id="pDone">' + _t("btnOk") + '</button></div>');
  sheet.querySelector("#pDone").addEventListener("click", closeSheet);
}
